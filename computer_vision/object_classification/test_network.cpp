...
int main()
{
  ...
  vector<size_t> currentLabel = {347, 279, 791, 4, 566, 941, 689, 381, 448, 977};
  vector<size_t> repeatLabel = {8, 8, 7, 8, 8, 8, 8, 8, 8, 8};
  ...
  for (int i = 0; i < input.n_cols; i++)
  {
    output.clear();
    model.Predict(input.col(i), output);
    size_t correctLabel = currentLabel[currentClass];
    repeatLabel[currentClass]--;
    if (repeatLabel[currentClass] == 0)
        currentClass++;
    if (correctLabel == output.col(0).index_max())
      correct++;
    sum = arma::accu(output);
    std::cout << std::setprecision(10) << sum << " --> " << output.col(0).index_max() <<
      "  " << output.col(0).max() << " --> Correct: " << correctLabel << std::endl;
  }

  std::cout << " " <<  (1.0 * correct / 76) << std::endl;
  return 0;
}